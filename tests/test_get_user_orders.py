...
class TestGetUserOrders:
    ...
    def test_get_orders_authorized(self):
        ...
        response = helpers.get_user_orders(user_with_orders["accessToken"])
        assert response.status_code == 200
        data = response.json()

        returned_ingredients = helpers.extract_ingredients_from_orders(data)

        # проверка success, количества заказов и совпадения ингредиентов
        assert (
                data["success"]
    ...
    def test_get_orders_unauthorized(self):
        response = helpers.get_user_orders(None)
